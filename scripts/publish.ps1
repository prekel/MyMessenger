...
if ($env:APPVEYOR_REPO_TAG.Contains("true"))
{
	...
	dotnet publish -c Release -r win-x64 -p:AssemblyVersion="$env:APPVEYOR_BUILD_VERSION" -o "$temp" -p:Version="$version" MyMessenger.Client.Console/MyMessenger.Client.Console.csproj
